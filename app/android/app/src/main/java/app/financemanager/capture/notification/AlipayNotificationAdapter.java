 ...
 * 典型通知格式：
 *   标题: "支付宝" / "支付宝到账"
 *   正文: "您的一笔付款已完成，金额 48.50 元，收款方:海底捞"
 *   或: "支付宝转账 48.50 元已到达"
 */
public class AlipayNotificationAdapter implements NotificationParserAdapter {

    private static final String TAG = "AlipayNotif";

    private static final Pattern AMOUNT_PATTERN = Pattern.compile("[¥￥]\\s*(\\d+[\\.\\d]*)|\\d+[\\.\\d]*\\s*元");
    private static final Pattern MERCHANT_PATTERN = Pattern.compile("(?:收款方|商家|商户|对方)[:：]\\s*(.+)");
    private static final Pattern ORDER_PATTERN = Pattern.compile("(?:订单号|交易号)[:：]\\s*(\\S+)");
    ...
    public CapturedPaymentInfo parse(String title, String text, String packageName) {
        ...
        if (!info.isHasAmount()) {
            Log.d(TAG, "支付宝通知未提取到金额，跳过");
            return null;
        }

        Log.d(TAG, "支付宝通知解析成功: " + info);
        return info;
    }
}